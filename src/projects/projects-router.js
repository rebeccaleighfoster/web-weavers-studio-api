...
projectsRouter.get("/:weaver_id", (req, res) => {
  ProjectsService.getByWeaverId(req.app.get("db"), req.params.weaver_id).then(
    (weaversById) => {
...
projectsRouter.get("/project/:project_id", (req, res) => {
  ProjectsService.getByProjectId(req.app.get("db"), req.params.project_id).then(
    (projectById) => {
...
projectsRouter.delete("/:id", (req, res, next) => {
  ProjectsService.deleteProjects(req.app.get("db"), req.params.id)
    .then(() => {
      res.status(204).end();
    })
    .catch(next);
